 ...
 * Modified by Kyomin Ku, 5521999, 23.10.2014 <kku031@aucklanduni.ac.nz>
 */
public class MandelModel {
	private static final int WMAX = 10;

	private int maxIter;
	private final List<MandelModelChangeListener> listeners = new ArrayList<MandelModelChangeListener>();
	...
	private ArrayList<SwingWorker<Void, Void>> workers = new ArrayList<SwingWorker<Void, Void>>();
		...
		for(int widx = 0; widx < WMAX ; widx++ ){
			...
			SwingWorker<Void,Void> worker = new SwingWorker<Void,Void>() {
				...
				public Void doInBackground() throws InterruptedException{
					...
					for (int bsize = 64; bsize >= 1; bsize = bsize/2){
						for (int x = xmin; x <= xmax; x += bsize) {
							for (int y = 0; y <= height; y += bsize) {
								...
								for (int b = x; (b < x+bsize) && (b < xmax); b++) {					
									for (int a = y; (a < y+bsize) && (a < height); a++) {
										iters[b*height + a] = escapeIters;
									}
								}
							}
						}
